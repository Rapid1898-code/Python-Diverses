...
# Input: INDEX_NAME: Name des Index der gelesen werden soll
# Input: CHAR: Ab welchen 2 Buchstaben die Titel aus dem Index gelesen werden soll - Standardwert ist "00" wenn nichts angegeben wird
...
def read_bilanz(stock):
    ...
    for i in range (2, len (output_gesamt[len (output_gesamt) - 1]) - 1):
        for j in  range (row_add):
            ...
            if j == 7 and rows[j] != []:
                if output_gesamt[ebit_row][i] not in [""," ","-"] and output_gesamt[totalassets_row][i] not in [""," ","-"] and output_gesamt[shorttermdebt_row][i] not in [""," ","-"]:
...
if index == 0: filename = "Stock_Data.xlsx"
else: filename = index.replace ("/", "_").replace ("kursliste", "") + "_Stock_Data_" + whg + ".xlsx"
for i, stock in enumerate(stocks_dic):
    if sek != 0:
        if i%30 == 0: vpn_land = vpn_switch (sek)
        print ("Verarbeitung:", stock, "with VPN:", vpn_land,"Daten lesen...")
    else:
        print ("Verarbeitung:", stock, "without VPN... Daten lesen...")
