...
# Combine Data from the 2 xlsx Price and Data
def combine_price_and_data(filename):
    fn_price = filename
    fn_data = filename.replace("Stock_Prices","Stock_Data")
    ...
    for sh_price in wb_price:
        ...
        for i in range (1,len(price_list)):
            ...
            if datetime.datetime.strptime(price_list[i][0],"%d.%m.%Y") != tmp_year:
                for j in range(2,len(row_title)):
                    if row_title[j] == "": continue
                    if datetime.datetime.strptime(price_list[i][0],"%d.%m.%Y").year-1 == int(row_title[j]):
...
print("Combining XLSX...")
combine_price_and_data(filename)
print("DONE")
print("Sort XLSX...")
sort_xlsx(filename)
print("DONE")
